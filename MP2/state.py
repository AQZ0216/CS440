...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        if self.zero_loc[0] < 2:
            ...
            potential_nbr[self.zero_loc[0]][self.zero_loc[1]] = self.state[self.zero_loc[0]+1][self.zero_loc[1]]
            potential_nbr[self.zero_loc[0]+1][self.zero_loc[1]] = 0
            new_state = EightPuzzleState(potential_nbr, self.goal, 
                                        dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, 
                                        zero_loc=[self.zero_loc[0]+1, self.zero_loc[1]])
            nbr_states.append(new_state)

        if self.zero_loc[1] > 0:
            potential_nbr = copy.deepcopy(self.state)
            potential_nbr[self.zero_loc[0]][self.zero_loc[1]] = self.state[self.zero_loc[0]][self.zero_loc[1]-1]
            potential_nbr[self.zero_loc[0]][self.zero_loc[1]-1] = 0
            new_state = EightPuzzleState(potential_nbr, self.goal, 
                                        dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, 
                                        zero_loc=[self.zero_loc[0], self.zero_loc[1]-1])
            nbr_states.append(new_state)
    ...
    def compute_heuristic(self):
        ...
        for i in range(3):
            for j in range(3):
                if self.state[i][j] != 0:
                    total += manhattan((i, j), (int(self.state[i][j]/3), self.state[i][j]%3))
        return total
...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for potential_nbr in neighboring_grid_locs:
            new_state = SingleGoalGridState(potential_nbr, self.goal, 
                                        dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, 
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for potential_nbr in neighboring_locs:
            ...
            for i in range(len(self.goal)):
                if potential_nbr == self.goal[i]:
                    new_goal = new_goal[:i] + new_goal[i+1:]
                    break

            new_state = GridState(potential_nbr, new_goal, 
                                        dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, 
    ...
    def compute_heuristic(self):
        if not self.goal:
            ...
            for i in self.goal:
                if (min > manhattan(self.state, i)):
                    min = manhattan(self.state, i)
            return min + self.mst_cache[self.goal]
