...
def transformToMaze(alien, goals, walls, window,granularity):
    ...
    input_map = [[[' ' for k in range(3)] for j in range(int(window[1]/granularity)+1)] for i in range(int(window[0]/granularity)+1)]

    idx = configToIdx(alien.get_config(), [0, 0, 0], granularity, alien)

    for i in range(int(window[0]/granularity)+1):
        for j in range(int(window[1]/granularity)+1):
            for k in range(3):
                alien.set_alien_config(idxToConfig([i, j, k], [0, 0, 0], granularity, alien))
    ...
    input_map[idx[0]][idx[1]][idx[2]] = 'P'

    return Maze(input_map, alien=alien, granularity=granularity)
