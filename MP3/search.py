...
def astar(maze, ispart1=False):
    ...
    while(frontier):
        ...
        for i in state.get_neighbors(ispart1=ispart1):       
            if (i not in visited_states or visited_states[i][1] > i.dist_from_start):
                heapq.heappush(frontier, i)
                visited_states[i] = (state, i.dist_from_start)
